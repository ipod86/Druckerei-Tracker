...
router.get('/group/:fromGroupId', requireAuth, (req, res) => {
  const fields = db.prepare(`
    SELECT tf.*, t.name as transition_name, t.id as transition_id,
           g1.name as from_group_name
    FROM transition_fields tf
    JOIN transitions t ON tf.transition_id = t.id
    JOIN groups g1 ON t.from_group_id = g1.id
    WHERE t.from_group_id = ?
    ORDER BY t.order_index, t.id, tf.order_index
  `).all(req.params.fromGroupId);
...
router.post('/', requireAdmin, (req, res) => {
  const { name, from_group_id } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO transitions (name, from_group_id, order_index)
    VALUES (?, ?, ?)
  `).run(name, from_group_id, (maxOrder.mx || 0) + 1);
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(name || null, from_group_id || null, req.params.id);
...
router.post('/:id/fields', requireAdmin, (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO transition_fields (transition_id, from_group_id, field_name, field_type, field_options, required, order_index)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    t.id,
    t.from_group_id,
